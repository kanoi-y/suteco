...
function createDisposalRule(
  overrides?: Partial<DisposalRule>
): DisposalRule {
  return {
...
describe("DisposalRuleRepository", () => {
  ...
  describe("save", () => {
    it("分別ルールを保存できる", async () => {
      ...
      const found = await repository.findByMunicipalityAndItem(
        rule.municipalityId,
        rule.itemId
      );
  ...
  describe("findByMunicipalityId", () => {
    it("自治体IDで該当ルールを全て取得できる", async () => {
      await repository.save(
        createDisposalRule({ itemId: "item_a", categoryName: "資源物" })
      );
      await repository.save(
        createDisposalRule({ itemId: "item_b", categoryName: "燃やすごみ" })
      );
      await repository.save(
        createDisposalRule({
      ...
      expect(found.map((r) => r.itemId)).toEqual(
        expect.arrayContaining(["item_a", "item_b"])
      );
    });
