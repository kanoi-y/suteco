...
describe("importDataset", () => {
  describe("正常 import テスト", () => {
    it("データセットを正常に取り込み、各テーブルに正しく保存される", async () => {
      ...
      expect(JSON.parse(items[0]?.aliases_json ?? "[]")).toEqual([
        "エイリアスA",
      ]);
      expect(JSON.parse(items[0]?.keywords_json ?? "[]")).toEqual([
        "キーワードA",
      ]);
  ...
  describe("冪等性テスト", () => {
    it("同じデータセットを2回取り込んでも、重複せず同じレコード数になる", async () => {
      ...
      expect(countAfterSecond.municipalities).toBe(
        countAfterFirst.municipalities,
      );
      expect(countAfterSecond.items).toBe(countAfterFirst.items);
      expect(countAfterSecond.disposal_rules).toBe(
        countAfterFirst.disposal_rules,
      );
